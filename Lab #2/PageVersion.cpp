...
Description Allocator::defineCategory(size_t s){
    Description pd;
    if (s > (pageSize >> 1)){
        pd.state = 2;
        pd.bsize = s / pageSize;
...
Allocator::Allocator(const size_t n, const size_t ps){
	pages = n / ps;
	size_t pds = sizeof(Description)/sizeof(size_t);
	cout << pds << endl;
	...
	pageSize = ps;
	pageDescriptors = (Description*)begin;
...
void* Allocator::mem_alloc(size_t s){
    Description def = defineCategory(s);
	size_t* res;
	if(def.state == 1){
...
size_t* Allocator::getFreeMBlock(size_t ps){
    Description pd = pageDescriptors[firstFreePage];
...
bool Allocator::freeLBlockIsLast(Description pd){
    LeftBD* desc = pd.firstFree;
...
size_t* Allocator::getFreeLBlock(size_t bs){
    ...
    Description pd = pageDescriptors[index];
    LeftBD* numberOfBlock = pd.firstFree;
	if(numberOfBlock->nextFreeBlock == 0){
		pd.firstFree = NULL;
		lBlocks[index] = pages+1;
	} else {
		pd.firstFree = (LeftBD*)((size_t*)(numberOfBlock) + numberOfBlock->nextFreeBlock);
	}
...
size_t Allocator::createLBlockPage(size_t bs){
	...
	if(index <= pages){
		...
		for(int i = 0; i < pages/bs; i++){
            LeftBD* des = (LeftBD*)addr[i*pd.bsize];
			des->nextFreeBlock = 1;
			if(i == pages/bs - 1)
				des->nextFreeBlock = 0;
		}
...
void Allocator::setAllFree(Description pd){
	size_t bs = pd.bsize;
